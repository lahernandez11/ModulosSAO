...
require_once 'models/PropuestaConciliacion.class.php';
...
try
{
	...
	switch ( $_REQUEST['action'] )
	{
		case 'eliminaTransaccion':
			...
			$transaccion = new PropuestaConciliacion($obra, $id_transaccion);
		...
		case 'getConceptosNuevoAvance':
			...
			$conceptos = PropuestaConciliacion::getConceptosNuevoAvance($obra, $id_concepto_raiz);
		...
		case 'getDatosTransaccion':
			...
			$data['datos']['confirmada'] = Util::formatoFecha($transaccion->confirmada());
		...
		case 'guardaTransaccion':
			...
			{
				$transaccion = new PropuestaConciliacion($obra, (int) $_POST['id_transaccion']);
			...
			{
				$transaccion = new PropuestaConciliacion(
						$obra,
						$fecha,
						$fechaInicio,
						$fechaTermino,
		...
		case 'confirmaTransaccion':
			$conn = SAODBConnFactory::getInstance($_POST['base_datos']);
			$obra = new Obra($conn, $_POST['id_obra']);
			$id_transaccion = (int) $_POST['id_transaccion'];

			$transaccion = new PropuestaConciliacion($obra, $id_transaccion);
			$transaccion->confirmar(Sesion::getUser());
			break;
		...
		case 'getFoliosTransaccion':
			...
			$folios = PropuestaConciliacion::getFoliosTransaccion($obra);
		...
		case 'getListaTransacciones':
			...
			$listaTran = PropuestaConciliacion::getListaTransacciones($obra);
