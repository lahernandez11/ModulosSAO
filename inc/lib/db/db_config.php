<?php
//Datos de configuración de la conexión a la base de datos
$dbServer['SAO1814'] 	 = '(local)\SQL2012';
$dbUser['SAO1814']   	 = 'App_ModulosSAO';
$userPassword['SAO1814'] = '@msApp85%';
$dbName['SAO1814'] 		 = 'SAO1814';

$dbServer['ModulosSAO'] 	 = '(local)\SQL2012';
$dbUser['ModulosSAO']   	 = 'App_ModulosSAO';
$userPassword['ModulosSAO']  = '@msApp85%';
$dbName['ModulosSAO'] 		 = 'ModulosSAO';

$dbServer['ReportesSAO'] 	 = '(local)\SQL2012';
$dbUser['ReportesSAO']   	 = 'App_ModulosSAO';
$userPassword['ReportesSAO'] = '@msApp85%';
$dbName['ReportesSAO'] 		 = 'ReportesSAO';

$dbServer['SAO1814_SUB'] 	 = '172.20.74.3';
$dbUser['SAO1814_SUB']   	 = 'App_ModulosSAO';
$userPassword['SAO1814_SUB'] = '@msApp85%';
$dbName['SAO1814_SUB'] 		 = 'SAO1814_SUB';
?>