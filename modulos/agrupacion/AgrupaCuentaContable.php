...
if( !isset($_POST['id']) ) {
	$data['errorMessage'] = 'No se especifico una cuenta contable.';
	echo json_encode($data);
	return;
}

if( !isset($_POST['idAgrupador']) ) {
	$data['errorMessage'] = 'No se especifico un agrupador para la cuenta.';
	echo json_encode($data);
	return;
}
...
$tsql = "{call [Agrupadores].[uspAgrupaCuentaContpaq]( ?, ?, ? )}";
