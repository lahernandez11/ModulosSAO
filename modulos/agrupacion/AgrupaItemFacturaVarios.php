...
if( ! isset($_POST['id']) ) {
	$data['errorMessage'] = 'No se especificó un item.';
	echo json_encode($data);
	return;
}

if( ! isset($_POST['idAgrupador']) ) {
	$data['errorMessage'] = 'No se especificó un agrupador para el item.';
	echo json_encode($data);
	return;
}
...
$tsql = "{call [AgrupacionFactVarios].[uspAgrupaItemFactVarios]( ?, ?, ?, ? )}";

$params = array(

	  array( $_POST['idProyecto'] , SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_INT)
	, array( $_POST['idTransaccion'] , SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_INT)
