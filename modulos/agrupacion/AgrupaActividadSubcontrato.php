...
if( !isset($_POST['id']) ) {
	$data['errorMessage'] = 'No se especifico una actividad.';
	echo json_encode($data);
	return;
}

if( !isset($_POST['idAgrupador']) ) {
	$data['errorMessage'] = 'No se especifico un agrupador para la actividad.';
	echo json_encode($data);
	return;
}
...
$tsql = "{call [AgrupacionSubcontratos].[uspAgrupaActividadSubcontrato]( ?, ?, ?, ?, ? )}";

$params = array(
				  array($_POST['idProyecto'], SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_INT)
				, array($_POST['idContratista'], SQLSRV_PARAM_IN, null, SQLSRV_SQLTYPE_INT)
